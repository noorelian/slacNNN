...
false_quenchfile = 'ACCL_L3B_3180_20221108_212822_QUENCH.txt'   # file with false quench waveform data
...
def plot_quench_waveforms(wf):
    ...
    if 'fault_waveform' in trimmed:
        ...
        plt.plot(trimmed['fault_time'], trimmed['fault_waveform'], label='Cavity quench waveform', **style)
    if 'decay_reference' in trimmed:
        style = line_styles.get('decay_reference', {})
        plt.plot(trimmed['fault_time'], trimmed['decay_reference'], label='Reference decay waveform', **style)
...
plt.savefig('quench_rfoff_compare_plot.pdf', dpi=300)